...
def test_owner_nest_content(url_nest, created_owner_client, created_nest_area):
    response = created_owner_client.get(url_nest)
    response_json = response.json()
    assert response_json['name'] == conftest.CREATED_NEST_NAME
    assert (response_json['new_creature_birth_process']
            == conftest.CREATED_NEST_B_PROCESS)
    assert response_json['area']['id'] == created_nest_area.id
